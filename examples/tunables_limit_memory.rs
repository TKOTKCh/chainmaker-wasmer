...
use wasmer::{imports, sys::{
    vm::{VMMemory, VMMemoryDefinition, VMTable, VMTableDefinition},
    BaseTunables, NativeEngineExt, Target, Tunables,
}, wat2wasm, AsStoreMut, Engine, Instance, Memory, MemoryError, MemoryStyle, MemoryType, Module, Pages, Store, TableStyle, TableType};
...
impl<T: Tunables> LimitingTunables<T> {
    ...
    fn adjust_memory(&self, requested: &MemoryType) -> MemoryType {
        ...
        if requested.maximum.is_none() {
            ...
            if adjusted.maximum.unwrap() > self.limit {
                adjusted.maximum = Some(self.limit);
            }
        }
        adjusted
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Max Memory of this instance: {:?}", first_memory.ty(&store).maximum.unwrap());
