...
#[derive(Clone)]
pub struct MeteringOpcodeCounters {
    counters: [u64; u8::MAX as usize + 1],
}

impl MeteringOpcodeCounters {
    pub fn new() -> Self {
        Self {
            counters: [0; u8::MAX as usize + 1],
        }
    }

    pub fn increment(&mut self, op: u8) {
        self.counters[op as usize] += 1;
    }

    pub fn get(&self, op: u8) -> u64 {
        self.counters[op as usize]
    }

    pub fn reset(&mut self) {
        self.counters = [0; u8::MAX as usize + 1];
    }
}
...
pub struct Metering<F: Fn(&Operator) -> u64 + Send + Sync> {
    ...
    opcode_counters: Mutex<MeteringOpcodeCounters>,
}
...
pub struct FunctionMetering<F: Fn(&Operator) -> u64 + Send + Sync> {
    ...
    opcode_counters: MeteringOpcodeCounters,
}
...
impl<F: Fn(&Operator) -> u64 + Send + Sync> Metering<F> {
    ...
    pub fn new(initial_limit: u64, cost_function: F) -> Self {
        Self {
            ...
            opcode_counters: Mutex::new(MeteringOpcodeCounters::new()), // 初始化
        }
    }

    // 新增：获取操作码计数器
    pub fn get_opcode_counters(&self) -> MeteringOpcodeCounters {
        self.opcode_counters.lock().unwrap().clone()
    }
}
...
impl<F: Fn(&Operator) -> u64 + Send + Sync> fmt::Debug for Metering<F> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("Metering")
            ...
            .field("opcode_counters", &self.opcode_counters)
            .finish()
    }
}
...
impl<F: Fn(&Operator) -> u64 + Send + Sync + 'static> ModuleMiddleware for Metering<F> {
    ...
    fn generate_function_middleware(&self, _: LocalFunctionIndex) -> Box<dyn FunctionMiddleware> {
        Box::new(FunctionMetering {
            ...
            opcode_counters: self.opcode_counters.lock().unwrap().clone(),
            accumulated_cost: 0,
        })
    }
...
impl<F: Fn(&Operator) -> u64 + Send + Sync> FunctionMiddleware for FunctionMetering<F> {
    fn feed<'a>(
        ...
        // 新增：更新操作码计数器
        let opcode = operator_to_u8(&operator);
        self.opcode_counters.increment(opcode);
...
// 辅助函数：将 Operator 转换为 u8 标识符
pub fn operator_to_u8(op: &Operator) -> u8 {
    use Operator::*;
    match op {
        Unreachable => 0x00,
        Nop => 0x01,
