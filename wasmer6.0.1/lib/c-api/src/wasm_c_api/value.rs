...
mod tests {
    ...
    use crate::wasm_c_api::value::{wasm_val_inner, wasm_val_t};
    use super::{
        wasm_val_vec_t,
        // wasm_val_vec_new,
        // wasm_val_vec_delete,
    ...
    fn test_memory_leak() {
        ...
        // 进入测试
        print("\n executing ");
        call_multi_times(&vals, num_vals);
    ...
    fn call_multi_times(values: &Vec<wasm_val_t>, num_values: usize) {
        for _ in 0..100_000 {
            print(". ");
            let mut wasm_val_vec = wasm_val_vec_t {
                size: num_values,
            ...
            unsafe {
                ...
                {
                    let ptr = &val_slice[0] as *const wasm_val_t;
                    let val_vec: Vec<wasm_val_t> = (0..num_values).map(|i| ptr.add(i).read()).collect();
                    wasm_val_vec.set_buffer(val_vec);
                }
